locals {
  ...
  vpc_id = var.create_vpc ? try(aws_vpc_ipv4_cidr_block_association.this[0].vpc_id, aws_vpc.this[0].id, "") : var.vpc_id
...
resource "aws_route" "public_internet_gateway" {
  count = (local.create_public_subnets && var.create_igw) || var.default_internet_gateway_id != null ? 1 : 0
  route_table_id         = try(var.default_route_table_id ,aws_route_table.public[0].id)
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = try(var.default_internet_gateway_id,aws_internet_gateway.this[0].id)
...
resource "aws_route" "public_internet_gateway_ipv6" {
  count = (local.create_public_subnets && var.create_igw && var.enable_ipv6) || ( var.default_internet_gateway_id != null && var.enable_ipv6 ) ? 1 : 0
...
resource "aws_route" "private_ipv6_egress" {
  count = var.create_egress_only_igw && var.enable_ipv6 && local.len_private_subnets > 0 ? local.nat_gateway_count : 0
